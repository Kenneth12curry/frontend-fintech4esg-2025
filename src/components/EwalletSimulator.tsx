...
export default function EWalletSimulatorApp() {
    ...
    <div className="flex justify-between items-center mt-3 p-4 border-b border-gray-200">
      {screen !== "home" ? (
        ...
        >
          ...
          {/* <span>{t("ewallet.simulator.back")}</span> */}
        </button>
      ) : (
      ...
      <div className="absolute text-purple-700 mt-3 font-bold left-0 right-0 flex justify-center pointer-events-none">
        <div className="font-medium select-none">
          {screen === "home" ? (
            <span className="text-purple-700 font-bold tracking-wide">ReadyPay</span>
  ...
  const renderHomeScreen = () => (
    <div className="flex flex-col h-full">
      {/* Balance Card */}
      <div className="p-4 bg-gradient-to-r from-purple-600 to-purple-800 text-white rounded-lg m-4">
        <div className="text-sm mb-1 opacity-80">{t("ewallet.simulator.balance.title")}</div>
        ...
        <div className="flex space-x-2">
          ...
          >
            ...
            {t("ewallet.simulator.balance.topup")}
          </Button>
          <Button 
            size="sm" 
            className="text-xs bg-white bg-opacity-20 hover:bg-opacity-30"
          ...
          >
            ...
            {t("ewallet.simulator.balance.send")}
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 px-4 mb-4">
        ...
        >
          ...
          <span className="text-xs text-center">{t("ewallet.simulator.actions.qrcode")}</span>
        </button>
        ...
        >
          ...
          <span className="text-xs text-center">{t("ewallet.simulator.actions.pay")}</span>
        </button>
        ...
        >
          ...
          <span className="text-xs text-center">{t("ewallet.simulator.actions.roundup")}</span>
        </button>
      </div>
  ...
  const renderQRCodeScreen = () => (
    <div className="flex flex-col items-center p-4">
      ...
      <div className="text-sm text-center text-gray-500 mb-4">
        {t("ewallet.simulator.qrcode.instruction")}
      </div>
      ...
      <Button
        ...
        onClick={goBack}
      >
        {t("ewallet.simulator.qrcode.done")}
      </Button>
    </div>
  );
  ...
  const renderTransferScreen = () => (
    <div className="p-4">
      <div className="bg-white rounded-lg p-4 shadow-sm mb-4">
        <div className="text-sm text-gray-500 mb-1">{t("ewallet.simulator.transfer.available")}</div>
        <div className="text-xl font-bold mb-4">{formatLocal(balance)}</div>
        
        <div className="space-y-4">
          <div>
            <label className="text-sm text-gray-500 block mb-1">{t("ewallet.simulator.transfer.recipient")}</label>
            <input 
              type="text" 
              className="w-full p-2 border border-gray-300 rounded-md"
              placeholder={t("ewallet.simulator.transfer.recipient_placeholder")}
              defaultValue="Kwame Nkrumah"
            />
          </div>
          
          <div>
            <label className="text-sm text-gray-500 block mb-1">{t("ewallet.simulator.transfer.amount")}</label>
          ...
          <div>
            <label className="text-sm text-gray-500 block mb-1">{t("ewallet.simulator.transfer.note")}</label>
            <input 
              type="text" 
              className="w-full p-2 border border-gray-300 rounded-md"
              placeholder={t("ewallet.simulator.transfer.note_placeholder")}
              defaultValue={t("ewallet.simulator.transfer.note_example")}
            />
          </div>
        </div>
      </div>
      ...
      >
        {t("ewallet.simulator.transfer.send")}
      </Button>
    </div>
  );
  ...
  const renderTopUpScreen = () => (
    <div className="p-4">
      <div className="bg-white rounded-lg p-4 shadow-sm mb-4">
        <div className="text-sm text-gray-500 mb-1">{t("ewallet.simulator.topup.current")}</div>
        <div className="text-xl font-bold mb-4">{formatLocal(balance)}</div>
        
        <div className="space-y-4">
          <div>
            <label className="text-sm text-gray-500 block mb-1">{t("ewallet.simulator.topup.method")}</label>
            <select className="w-full p-2 border border-gray-300 rounded-md">
              <option value="mobile">{t("ewallet.simulator.topup.mobile_money")}</option>
              <option value="bank">{t("ewallet.simulator.topup.bank")}</option>
              <option value="card">{t("ewallet.simulator.topup.card")}</option>
            </select>
          </div>
          
          <div>
            <label className="text-sm text-gray-500 block mb-1">{t("ewallet.simulator.topup.amount")}</label>
            <div className="relative">
              <span className="absolute left-3 top-2 text-gray-500">{symbol}</span>
              <input 
                type="number" 
      ...
      <Button 
        ...
        onClick={handleTopUp}
      >
        {t("ewallet.simulator.topup.confirm")}
      </Button>
    </div>
  );
  ...
  const renderHistoryScreen = () => (
    <div className="p-4">
      <div className="space-y-3">
        {transactions.map((tx) => (
          <div key={tx.id} className="flex justify-between items-center p-3 bg-white shadow-sm rounded-lg">
            <div className="flex items-center">
      ...
      <div className="space-y-4">
        ...
        >
          <div className="bg-gradient-to-r from-purple-500 to-purple-700 p-3 text-white">
            <div className="flex items-center">
              ...
              <span className="font-medium">EduSave Layaway</span>
            </div>
          </div>
          <div className="p-4">
            <div className="mb-3">
        ...
        >
          <div className="bg-gradient-to-r from-blue-500 to-blue-700 p-3 text-white">
            <div className="flex items-center">
              <div className="mr-3 p-2 bg-blue-400 bg-opacity-30 rounded-full">
        ...
        >
          ...
          <div className="p-4">
            <div className="mb-3">
              ...
              <div className="text-lg font-medium">{formatLocal(1250)}</div>
            </div>
            <div className="mb-3">
              <div className="text-sm text-gray-500 mb-1">Monthly Interest</div>
              <div className="text-lg font-medium">3.2% <span className="text-sm font-normal text-gray-500">flat rate</span></div>
            </div>
            <div className="text-xs text-gray-500 mt-3">
        ...
        >
          <div className="bg-gradient-to-r from-blue-500 to-blue-700 p-3 text-white">
            <div className="flex items-center">
              ...
              <span className="font-medium">Business Boost Loan</span>
            </div>
          </div>
          <div className="p-4">
            <div className="mb-3">
              <div className="text-sm text-gray-500 mb-1">Maximum Amount</div>
              <div className="text-lg font-medium">{formatLocal(15000)}</div>
            </div>
            <div className="mb-3">
              <div className="text-sm text-gray-500 mb-1">Repayment Period</div>
              <div className="text-lg font-medium">6-18 <span className="text-sm font-normal text-gray-500">months</span></div>
            </div>
            <div className="text-xs text-gray-500 mt-3">
      ...
      <div className="bg-white rounded-lg p-4 shadow-sm mb-4">
        ...
        <div className="flex space-x-2 text-sm">
          <Button 
            ...
            onClick={goBack}
          >
            {t("ewallet.simulator.roundup.configure")}
          </Button>
          <Button 
            variant="default" 
            size="sm"
          ...
          >
            ...
            {t("ewallet.simulator.roundup.refresh")}
          </Button>
        </div>
      </div>
      
      <div className="bg-white rounded-lg p-4 shadow-sm">
        <h4 className="font-medium text-sm mb-2">{t("ewallet.simulator.roundup.how_title")}</h4>
        <p className="text-xs text-gray-500 mb-2">{t("ewallet.simulator.roundup.how_description_1")}</p>
        <p className="text-xs text-gray-500">{t("ewallet.simulator.roundup.how_description_2")}</p>
      </div>
    </div>
  );
      ...
      <div className="space-y-4">
        <div className="bg-white rounded-lg overflow-hidden">
          <div className="p-4 border-b border-gray-100">
            <div className="text-sm font-medium">{t("ewallet.simulator.settings.account")}</div>
          </div>
          {["profile", "security", "notifications", "linked_accounts"].map((item) => (
            <div key={item} className="flex justify-between items-center p-4 border-b border-gray-100">
              <div className="text-sm">{t(`ewallet.simulator.settings.${item}`)}</div>
        ...
        <div className="bg-white rounded-lg overflow-hidden">
          <div className="p-4 border-b border-gray-100">
            <div className="text-sm font-medium">{t("ewallet.simulator.settings.preferences")}</div>
          </div>
        ...
        <div className="bg-white rounded-lg overflow-hidden">
          <div className="p-4 border-b border-gray-100">
            <div className="text-sm font-medium">{t("ewallet.simulator.settings.support")}</div>
          </div>
        ...
        <Button variant="destructive" className="w-full">
          {t("ewallet.simulator.settings.logout")}
        </Button>
      </div>
    </div>
  );
  ...
  const deviceWidth = isMobile ? "w-full max-w-[360px]" : "w-[300px]";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-center gap-8 mt-10">
          ...
          <AnimatedComponent animation="slideUp" delay={0.3} duration={0.6} className="w-full max-w-md">
            ...
            >
              <TabsList className="grid w-full grid-cols-2 mb-8">
                <TabsTrigger value="agent">Agent/Merchant Account</TabsTrigger>
                ...
                >
                  <div className="flex items-start mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600">
                        For Microfinance Institutions (MFIs) and Local Banks
                      </p>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-2 text-gray-600 mb-6">
                    ...
                    >
                      ...
                      <span className="text-sm">Account activation via the ReadyCash platform and application</span>
                    </motion.li>
                    <motion.li 
                      key={2} 
                      className="flex items-start"
                    ...
                    >
                      <div className="text-purple-600 mr-2 mt-0.5 flex-shrink-0">•</div>
                      <span className="text-sm">QR code functionality for customer transactions</span>
                    </motion.li>
                    <motion.li 
                      key={3} 
                      className="flex items-start"
                    ...
                    >
                      <div className="text-purple-600 mr-2 mt-0.5 flex-shrink-0">•</div>
                      <span className="text-sm">Accounting cash register with detailed transaction recording</span>
                    </motion.li>
                    <motion.li 
                      key={4} 
                      className="flex items-start"
                    ...
                    >
                      <div className="text-purple-600 mr-2 mt-0.5 flex-shrink-0">•</div>
                      <span className="text-sm">Secure cash control with customizable limits</span>
                    </motion.li>
                  </ul>
                </HoverAnimationCard>
              </TabsContent>
          ...
          <AnimatedComponent animation="slideUp" delay={0.5} duration={0.6} className="flex justify-center">
            <div 
              className={`${deviceWidth} h-[540px] bg-gray-100 rounded-[36px] border-[14px] border-gray-900 overflow-hidden relative shadow-xl`}
            >
              {/* Status Bar */}
